...
class DroneModel(Enum):
    """Drone ppo_models enumeration class."""
...
class BaseAviary(gym.Env):
        ...
        if self.VISION_ATTR:
            ...
            if self.RECORD:
                self.ONBOARD_IMG_PATH = os.path.dirname(
                    os.path.abspath(__file__)) + "/../../files/videos/onboard-" + datetime.now().strftime(
                    "%m.%d.%Y_%H.%M.%S") + "/"
    ...
    def step(self,
        ...
        for _ in range(self.AGGR_PHY_STEPS):
            ...
            if self.AGGR_PHY_STEPS > 1 and self.PHYSICS in [Physics.DYN, Physics.PYB_GND, Physics.PYB_DRAG,
        ...
        # 原来是先computeReward 再computeDone
    ...
    def _housekeeping(self):
        ...
        #### Load ground plane, drone and obstacles ppo_models #########
    ...
    def _startVideoRecording(self):
        ...
        if self.RECORD and self.GUI:
            self.VIDEO_ID = p.startStateLogging(loggingType=p.STATE_LOGGING_VIDEO_MP4,
                                                fileName=os.path.dirname(os.path.abspath(
                                                    __file__)) + "/../../files/videos/video-" + datetime.now().strftime(
                                                    "%m.%d.%Y_%H.%M.%S") + ".mp4",
                                                physicsClientId=self.CLIENT
                                                )
        if self.RECORD and not self.GUI:
            ...
            self.IMG_PATH = os.path.dirname(
                os.path.abspath(__file__)) + "/../../files/videos/video-" + datetime.now().strftime(
                "%m.%d.%Y_%H.%M.%S") + "/"
